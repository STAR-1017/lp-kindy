...
$(function(){
  ...
  $('a[href^="#"]').not(".js-pageTop a").click(function(){
    var elmID = ($(this).attr("href")) ? $(this).attr("href") : ".main";
...
$(function(){
  ...
  $(window).on('load resize scroll', function () {
    ...
    if (btnPageTop.length) {
      ...
      if (sclH - sclPosi <= pageFtrH - ftrBtmMove) {
        btnPageTop.addClass('is-fixed');
      } else {
...
jQuery(function($){
	$(".colorful").children().addBack().contents().each(function(){
      if (this.nodeType == 3) {
         var $this = $(this);
         $this.replaceWith($this.text().replace(/(\S)/g, "<span><b>$&</b></span>"));
      }
   });
	$('.point2_photo').jqFloat({
		width: 0,
		height: 20,
		speed: 1000
	});
	
	$('.point3_photo').jqFloat({
		width: 0,
		height: 20,
		speed: 1000
	});
	
	$('.point4_photo').jqFloat({
		width: 0,
		height: 20,
		speed: 1000
	});
	
	$('.point5_photo').jqFloat({
		width: 0,
		height: 20,
		speed: 1000
	});
	$(".mainvisual_img_box div:gt(0)").hide();
	$(window).load(function(){
		setInterval(function(){
			$(".mainvisual_img_box div:first-child").fadeOut(1500)
			.next("div").fadeIn(1500)
			.end().appendTo(".mainvisual_img_box");}, 
			3000);
	});
});
